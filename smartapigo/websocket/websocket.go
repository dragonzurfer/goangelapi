...
func (s *SocketClient) SetScrips(val string) {
	s.scrips = val
}
...
func (s *SocketClient) Subscribe(customscrips ...string) error {
	if s.Conn == nil {
		return errors.New("connection is closed: subscibe failed")
	}
	if len(customscrips) > 0 {
		s.scrips = customscrips[0]
	}
	s.SubscribeQueue()
...
func (s *SocketClient) SubscribeQueue(customscrips ...string) error {
	if len(customscrips) > 0 {
		s.scrips = customscrips[0]
	}
	fmt.Println("Subscribing Q")
	err := s.Conn.WriteMessage(websocket.TextMessage, []byte(`{"task":"dp","channel":"`+s.scrips+`","token":"`+s.feedToken+`","user": "`+s.clientCode+`","acctid":"`+s.clientCode+`"}`))
	if err != nil {
		s.triggerError(err)
		return err
	}
